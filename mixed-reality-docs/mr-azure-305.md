...
4.  <span data-ttu-id="1c3a2-164">Une fois que vous avez cliqué sur **créer**:</span><span class="sxs-lookup"><span data-stu-id="1c3a2-164">Once you have clicked on **Create**:</span></span>
    ...
    1.  <span data-ttu-id="1c3a2-165">Insérez un *nom* pour votre compte. n’oubliez pas que ce champ accepte uniquement des chiffres et des lettres minuscules.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-165">Insert a *Name* for your account, be aware this field only accepts numbers, and lowercase letters.</span></span>
    ...
    11. <span data-ttu-id="1c3a2-180">Sélectionnez **Créer**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-180">Select **Create**.</span></span>
...
5.  <span data-ttu-id="1c3a2-265">Accédez à **fichier** > **paramètres de build** et assurez-vous que :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-265">Go to **File** > **Build Settings** and make sure that:</span></span>
    ...
    4. <span data-ttu-id="1c3a2-270">**Version de Visual Studio** définie sur le **dernier installé**</span><span class="sxs-lookup"><span data-stu-id="1c3a2-270">**Visual Studio Version** is set to **Latest installed**</span></span>
...
8.  <span data-ttu-id="1c3a2-284">Dans ce volet, quelques paramètres doivent être vérifiés :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-284">In this panel, a few settings need to be verified:</span></span>
    ...
    2.  <span data-ttu-id="1c3a2-289">Dans l’onglet **paramètres de publication** , sous **fonctionnalités**, activez la case à cocher :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-289">Within the **Publishing Settings** tab, under **Capabilities**, check:</span></span>
...
1.  <span data-ttu-id="1c3a2-332">Cliquez avec le bouton droit dans une zone vide du *panneau hiérarchie*, sous **objet 3D**, ajoutez un **plan**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-332">Right-click in an empty area of the *Hierarchy Panel*, under **3D  Object**, add a **Plane**.</span></span>
...
7.  <span data-ttu-id="1c3a2-385">Renommez l’objet **3D Text** en **AzureStatusText**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-385">Rename the **3D Text** object to **AzureStatusText**.</span></span>
...
-   <span data-ttu-id="1c3a2-442">Stockage des informations d’identification du compte Azure.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-442">Storing Azure Account credentials.</span></span>
...
10. <span data-ttu-id="1c3a2-462">Enregistrez vos modifications dans Visual Studio, puis revenez à Unity.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-462">Save your changes in Visual Studio, and then return to Unity.</span></span>
...
3.  <span data-ttu-id="1c3a2-475">Double-cliquez sur le nouveau script *ShapeFactory* pour l’ouvrir avec *Visual Studio*.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-475">Double click on the new *ShapeFactory* script to open it with *Visual Studio*.</span></span>
...
9. <span data-ttu-id="1c3a2-483">Une fois l’appareil photo sélectionné, vous remarquerez que le composant script *ShapeFactory* ne contient pas la référence du *point de génération* .</span><span class="sxs-lookup"><span data-stu-id="1c3a2-483">With the Main Camera selected you will notice the *ShapeFactory* script component is missing the *Spawn Point* reference.</span></span> <span data-ttu-id="1c3a2-484">Pour résoudre ce problème, faites glisser l’objet **ShapeSpawnPoint** du volet de la *hiérarchie* vers la cible de référence de **point de génération** .</span><span class="sxs-lookup"><span data-stu-id="1c3a2-484">To fix it, drag the **ShapeSpawnPoint** object from the *Hierarchy Panel* to the **Spawn Point** reference target.</span></span>

    ![définir la cible de référence de fabrique de forme](images/AzureLabs-Lab5-53.png)

## <a name="chapter-9---create-the-gaze-class"></a><span data-ttu-id="1c3a2-486">Chapitre 9-créer la classe en regard</span><span class="sxs-lookup"><span data-stu-id="1c3a2-486">Chapter 9 - Create the Gaze class</span></span>

<span data-ttu-id="1c3a2-487">Le dernier script que vous devez créer est la classe de *regard* .</span><span class="sxs-lookup"><span data-stu-id="1c3a2-487">The last script you need to create is the *Gaze* class.</span></span>

<span data-ttu-id="1c3a2-488">Cette classe est chargée de créer un **Raycast** qui sera projeté à l’avance de la caméra principale, pour détecter l’objet que l’utilisateur examine.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-488">This class is responsible for creating a **Raycast** that will be projected forward from the Main Camera, to detect which object the user is looking at.</span></span> <span data-ttu-id="1c3a2-489">Dans ce cas, le Raycast doit déterminer si l’utilisateur regarde l’objet **GazeButton** dans la scène et déclencher un comportement.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-489">In this case, the Raycast will need to identify if the user is looking at the **GazeButton** object in the scene and trigger a behavior.</span></span>

<span data-ttu-id="1c3a2-490">Pour créer cette classe :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-490">To create this Class:</span></span>

1.  <span data-ttu-id="1c3a2-491">Accédez au dossier **scripts** que vous avez créé précédemment.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-491">Go to the **Scripts** folder you created previously.</span></span>

2.  <span data-ttu-id="1c3a2-492">Cliquez avec le bouton droit dans le panneau projet, puis **créez** >  **C# script**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-492">Right-click in the Project Panel, **Create** > **C# Script**.</span></span> <span data-ttu-id="1c3a2-493">Appelez le point de *regard*du script.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-493">Call the script *Gaze*.</span></span>

3.  <span data-ttu-id="1c3a2-494">Double-cliquez sur le nouveau script de *regard* pour l’ouvrir avec *Visual Studio.*</span><span class="sxs-lookup"><span data-stu-id="1c3a2-494">Double click on the new *Gaze* script to open it with *Visual Studio.*</span></span>

4.  <span data-ttu-id="1c3a2-495">Assurez-vous que l’espace de noms suivant est inclus en haut du script :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-495">Ensure the following namespace is included at the top of the script:</span></span>

    ```csharp
        using UnityEngine;
    ```

5.  <span data-ttu-id="1c3a2-496">Ajoutez ensuite les variables suivantes à l’intérieur de la classe *regard* :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-496">Then add the following variables inside the *Gaze* class:</span></span>
...
> [!IMPORTANT]
> <span data-ttu-id="1c3a2-497">Certaines de ces variables pourront être modifiées dans l' *éditeur*.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-497">Some of these variables will be able to be edited in the *Editor*.</span></span>

6.  <span data-ttu-id="1c3a2-498">Vous devez maintenant ajouter du code pour les méthodes *éveillé ()* et *Start ()* .</span><span class="sxs-lookup"><span data-stu-id="1c3a2-498">Code for the *Awake()* and *Start()* methods now needs to be added.</span></span>
...
8. <span data-ttu-id="1c3a2-500">Ensuite, ajoutez la méthode *UpdateRaycast ()* , qui projetera un Raycast et détectera la cible d’accès.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-500">Next add the *UpdateRaycast()* method, which will project a Raycast and detect the hit target.</span></span>
...
9. <span data-ttu-id="1c3a2-501">Enfin, ajoutez la méthode *ResetFocusedObject ()* , qui va basculer la couleur active des objets GazeButton, en indiquant si elle crée ou non une nouvelle forme.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-501">Lastly, add the *ResetFocusedObject()* method, which will toggle the GazeButton objects current color, indicating whether it is creating a new shape or not.</span></span>
...
10.  <span data-ttu-id="1c3a2-502">Enregistrez vos modifications dans Visual Studio avant de revenir à Unity.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-502">Save your changes in Visual Studio before returning to Unity.</span></span>

11.  <span data-ttu-id="1c3a2-503">Cliquez et faites glisser *la classe pointage du dossier* scripts vers l’objet **caméra principal** dans le *panneau hiérarchie*.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-503">Click and drag the *Gaze* class from the Scripts folder to the **Main Camera** object in the *Hierarchy Panel*.</span></span>

## <a name="chapter-10---completing-the-azureservices-class"></a><span data-ttu-id="1c3a2-504">Chapitre 10-réalisation de la classe AzureServices</span><span class="sxs-lookup"><span data-stu-id="1c3a2-504">Chapter 10 - Completing the AzureServices class</span></span>

<span data-ttu-id="1c3a2-505">Avec les autres scripts en place, il est désormais possible d' *effectuer* la classe *AzureServices* .</span><span class="sxs-lookup"><span data-stu-id="1c3a2-505">With the other scripts in place, it is now possible to *complete* the *AzureServices* class.</span></span> <span data-ttu-id="1c3a2-506">Pour ce faire, procédez comme suit :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-506">This will be achieved through:</span></span>

1.  <span data-ttu-id="1c3a2-507">Ajout d’une nouvelle méthode nommée *CreateCloudIdentityAsync ()* , pour configurer les variables d’authentification nécessaires à la communication avec Azure.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-507">Adding a new method named *CreateCloudIdentityAsync()*, to set up the authentication variables needed for communicating with Azure.</span></span>

    > <span data-ttu-id="1c3a2-508">Cette méthode vérifie également l’existence d’un fichier précédemment stocké contenant la liste de formes.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-508">This method will also check for the existence of a previously stored File containing the Shape List.</span></span>
    >
    > <span data-ttu-id="1c3a2-509">**Si le fichier est trouvé**, il désactive le point d’interdépendance de *l’utilisateur et*déclenche la création de la forme, en fonction du modèle de formes, tel qu’il est stocké dans le **fichier de stockage Azure**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-509">**If the file is found**, it will disable the user *Gaze*, and trigger Shape creation, according to the pattern of shapes, as stored in the **Azure Storage file**.</span></span> <span data-ttu-id="1c3a2-510">L’utilisateur peut voir cela, car la **maille de texte** fournira « stockage » ou « nouveau », en fonction de l’origine des formes.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-510">The user can see this, as the **Text Mesh** will provide display 'Storage' or 'New', depending on the shapes origin.</span></span>
    >
    > <span data-ttu-id="1c3a2-511">**Si aucun fichier n’est trouvé**, il active le point de *regard*, ce qui permet à l’utilisateur de créer des formes lors de la recherche de l’objet **GazeButton** dans la scène.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-511">**If no file is found**, it will enable the *Gaze*, enabling the user to create shapes when looking at the **GazeButton** object in the scene.</span></span>
...
2.  <span data-ttu-id="1c3a2-512">L’extrait de code suivant se trouve dans la méthode *Start ()* ; où un appel sera effectué à la méthode *CreateCloudIdentityAsync ()* .</span><span class="sxs-lookup"><span data-stu-id="1c3a2-512">The next code snippet is from within the *Start()* method; wherein a call will be made to the *CreateCloudIdentityAsync()* method.</span></span> <span data-ttu-id="1c3a2-513">N’hésitez pas à effectuer une copie sur votre méthode *Start ()* actuelle, avec les éléments suivants :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-513">Feel free to copy over your current *Start()* method, with the below:</span></span>
...
3.  <span data-ttu-id="1c3a2-514">Renseignez le code de la méthode *CallAzureFunctionForNextShape ()* .</span><span class="sxs-lookup"><span data-stu-id="1c3a2-514">Fill in the code for the method *CallAzureFunctionForNextShape()*.</span></span> <span data-ttu-id="1c3a2-515">Vous allez utiliser la *Function App Azure* précédemment créée pour demander un index de forme.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-515">You will use the previously created *Azure Function App* to request a shape index.</span></span> <span data-ttu-id="1c3a2-516">Une fois la nouvelle forme reçue, cette méthode envoie la forme à la classe *ShapeFactory* pour créer la nouvelle forme dans la scène.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-516">Once the new shape is received, this method will send the shape to the *ShapeFactory* class to create the new shape in the scene.</span></span> <span data-ttu-id="1c3a2-517">Utilisez le code ci-dessous pour terminer le corps de *CallAzureFunctionForNextShape ()* .</span><span class="sxs-lookup"><span data-stu-id="1c3a2-517">Use the code below to complete the body of *CallAzureFunctionForNextShape()*.</span></span>
...
4.  <span data-ttu-id="1c3a2-518">Ajoutez une méthode pour créer une chaîne en concaténant les entiers stockés dans la liste de l’historique des formes et en l’enregistrant dans votre *fichier de stockage Azure*.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-518">Add a method to create a string, by concatenating the integers stored in the shape history list, and saving it in your *Azure Storage File*.</span></span>
...
5.  <span data-ttu-id="1c3a2-519">Ajoutez une méthode pour récupérer le texte stocké dans le fichier situé dans votre *fichier de stockage Azure* et le *désérialiser* dans une liste.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-519">Add a method to retrieve the text stored in the file located in your *Azure Storage File* and *deserialize* it into a list.</span></span>

6.  <span data-ttu-id="1c3a2-520">Une fois ce processus terminé, la méthode réactive le point de regard pour permettre à l’utilisateur d’ajouter des formes à la scène.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-520">Once this process is completed, the method re-enables the gaze so that the user can add more shapes to the scene.</span></span>
...
7.  <span data-ttu-id="1c3a2-521">Enregistrez vos modifications dans Visual Studio avant de revenir à Unity.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-521">Save your changes in Visual Studio before returning to Unity.</span></span>

## <a name="chapter-11---build-the-uwp-solution"></a><span data-ttu-id="1c3a2-522">Chapitre 11-créer la solution UWP</span><span class="sxs-lookup"><span data-stu-id="1c3a2-522">Chapter 11 - Build the UWP Solution</span></span>

<span data-ttu-id="1c3a2-523">Pour commencer le processus de génération :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-523">To begin the Build process:</span></span>

1.  <span data-ttu-id="1c3a2-524">Accédez à **fichier** > **paramètres de build**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-524">Go to **File** > **Build Settings**.</span></span>

    ![générer l’application](images/AzureLabs-Lab5-54.png)

2.  <span data-ttu-id="1c3a2-526">Cliquez sur **Build**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-526">Click **Build**.</span></span> <span data-ttu-id="1c3a2-527">Unity lance une fenêtre de l' *Explorateur de fichiers* , dans laquelle vous devez créer, puis sélectionner un dossier dans lequel générer l’application.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-527">Unity will launch a *File Explorer* window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="1c3a2-528">Créez ce dossier maintenant, puis nommez-le *application*.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-528">Create that folder now, and name it *App*.</span></span> <span data-ttu-id="1c3a2-529">Ensuite, avec le dossier d' *application* sélectionné, appuyez sur **Sélectionner un dossier**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-529">Then with the *App* folder selected, press **Select Folder**.</span></span>

3.  <span data-ttu-id="1c3a2-530">Unity commence à générer votre projet dans le dossier de l' *application* .</span><span class="sxs-lookup"><span data-stu-id="1c3a2-530">Unity will begin building your project to the *App* folder.</span></span>

4.  <span data-ttu-id="1c3a2-531">Une fois la génération de Unity terminée (cela peut prendre un certain temps), une fenêtre de l' *Explorateur de fichiers* s’ouvre à l’emplacement de votre Build (Vérifiez la barre des tâches, car elle ne s’affiche pas toujours au-dessus de votre Windows, mais vous informera de l’ajout d’une nouvelle fenêtre).</span><span class="sxs-lookup"><span data-stu-id="1c3a2-531">Once Unity has finished building (it might take some time), it will open a *File Explorer* window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

## <a name="chapter-12---deploying-your-application"></a><span data-ttu-id="1c3a2-532">Chapitre 12 : déploiement de votre application</span><span class="sxs-lookup"><span data-stu-id="1c3a2-532">Chapter 12 - Deploying your application</span></span>

<span data-ttu-id="1c3a2-533">Pour déployer votre application :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-533">To deploy your application:</span></span>

1.  <span data-ttu-id="1c3a2-534">Accédez au dossier de l' *application* qui a été créé dans le [dernier chapitre](#chapter-11---build-the-uwp-solution).</span><span class="sxs-lookup"><span data-stu-id="1c3a2-534">Navigate to the *App* folder which was created in the [last Chapter](#chapter-11---build-the-uwp-solution).</span></span> <span data-ttu-id="1c3a2-535">Vous verrez un fichier avec le nom de votre application, avec l’extension « . sln », que vous devez double-cliquer, afin de l’ouvrir dans *Visual Studio*.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-535">You will see a file with your apps name, with the '.sln' extension, which you should double-click, so to open it within *Visual Studio*.</span></span>

2.  <span data-ttu-id="1c3a2-536">Dans la **plateforme**de la solution, sélectionnez **x86, ordinateur local**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-536">In the **Solution Platform**, select **x86, Local Machine**.</span></span>

3.  <span data-ttu-id="1c3a2-537">Dans la **configuration** de la solution, sélectionnez **Déboguer**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-537">In the **Solution Configuration** select **Debug**.</span></span>

    > <span data-ttu-id="1c3a2-538">Pour Microsoft HoloLens, il peut s’avérer plus facile de définir cette valeur sur *machine distante*, afin de ne pas être attaché à votre ordinateur.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-538">For the Microsoft HoloLens, you may find it easier to set this to *Remote Machine*, so that you are not tethered to your computer.</span></span> <span data-ttu-id="1c3a2-539">Toutefois, vous devez également effectuer les opérations suivantes :</span><span class="sxs-lookup"><span data-stu-id="1c3a2-539">Though, you will need to also do the following:</span></span>
    > - <span data-ttu-id="1c3a2-540">Identifiez l' **adresse IP** de votre HoloLens, qui se trouve dans les **paramètres** > **réseau & Internet** > les **Options avancées**du **Wi-Fi** > ; IPv4 est l’adresse que vous devez utiliser.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-540">Know the **IP Address** of your HoloLens, which can be found within the **Settings** > **Network & Internet** > **Wi-Fi** > **Advanced Options**; the IPv4 is the address you should use.</span></span> 
    > - <span data-ttu-id="1c3a2-541">Assurez-vous que le **mode développeur** est **activé**; trouvé dans **paramètres** > **mettre à jour &**  > **de sécurité pour les développeurs**.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-541">Ensure **Developer Mode** is **On**; found in **Settings** > **Update & Security** > **For developers**.</span></span>

    ![déployer la solution](images/AzureLabs-Lab5-55.png)

4.  <span data-ttu-id="1c3a2-543">Accédez au menu **générer** , puis cliquez sur **déployer la solution** pour chargement l’application sur votre ordinateur.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-543">Go to the **Build** menu and click on **Deploy Solution** to sideload the application to your machine.</span></span>

5.  <span data-ttu-id="1c3a2-544">Votre application doit maintenant apparaître dans la liste des applications installées, prêtes à être lancées et testées.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-544">Your App should now appear in the list of installed apps, ready to be launched and tested!</span></span>

## <a name="your-finished-azure-functions-and-storage-application"></a><span data-ttu-id="1c3a2-545">Votre application de Azure Functions et de stockage terminée</span><span class="sxs-lookup"><span data-stu-id="1c3a2-545">Your finished Azure Functions and Storage Application</span></span>

<span data-ttu-id="1c3a2-546">Félicitations, vous avez créé une application de réalité mixte qui tire parti des services de stockage Azure Functions et Azure.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-546">Congratulations, you built a mixed reality app that leverages both the Azure Functions and Azure Storage services.</span></span> <span data-ttu-id="1c3a2-547">Votre application sera en mesure de dessiner sur des données stockées et de fournir une action basée sur ces données.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-547">Your app will be able to draw on stored data, and provide an action based on that data.</span></span>

![fin du produit final](images/AzureLabs-Lab5-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="1c3a2-549">Exercices bonus</span><span class="sxs-lookup"><span data-stu-id="1c3a2-549">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="1c3a2-550">Exercice 1</span><span class="sxs-lookup"><span data-stu-id="1c3a2-550">Exercise 1</span></span>

<span data-ttu-id="1c3a2-551">Créez un deuxième point de lancement et un autre enregistrement à partir duquel un objet a été créé.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-551">Create a second spawn point and record which spawn point an object was created from.</span></span> <span data-ttu-id="1c3a2-552">Lorsque vous chargez le fichier de données, relisez les formes générées à partir de l’emplacement où elles ont été créées à l’origine.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-552">When you load the data file, replay the shapes being spawned from the location they originally were created.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="1c3a2-553">Exercice 2</span><span class="sxs-lookup"><span data-stu-id="1c3a2-553">Exercise 2</span></span>

<span data-ttu-id="1c3a2-554">Créez un moyen de redémarrer l’application, au lieu de la rouvrir à chaque fois.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-554">Create a way to restart the app, rather than having to re-open it each time.</span></span> <span data-ttu-id="1c3a2-555">Le **chargement des scènes** est un bon point de départ.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-555">**Loading Scenes** is a good spot to start.</span></span> <span data-ttu-id="1c3a2-556">Après cela, créez un moyen d’effacer la liste stockée dans le *stockage Azure*, afin qu’elle puisse être facilement réinitialisée à partir de votre application.</span><span class="sxs-lookup"><span data-stu-id="1c3a2-556">After doing that, create a way to clear the stored list in *Azure Storage*, so that it can be easily reset from your app.</span></span> 
